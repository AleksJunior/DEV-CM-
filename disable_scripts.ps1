...
.DESCRIPTION
    Устанавливает политику выполнения Restricted, которая запрещает запуск любых PowerShell сценариев.
    Также удаляет разблокировку скриптов (возвращает метку "скачано из интернета").
.NOTES
...
Write-Host ""
Write-Host "Продолжить? (y/n)" -ForegroundColor Cyan
