...
.SYNOPSIS
    GUI для управления сертификатами и компонентами Авест.
.DESCRIPTION
    Основной интерфейс. Кнопки:
    - Обновить сертификаты (из all_certs_urls.txt)
    - Установить компоненты Авест (AvPass, AvBign, AvCSPBel, AvCSPBign, AvReg)
    - Установить плагин AvCMXWebP (для IE Mode)
    - Запланировать автозапуск (планировщик, при старте системы)
    - URL сертификатов (открыть all_certs_urls.txt в блокноте)
    - Инфо о компонентах (статус из реестра)
    - Информация о плагине (версия, статус)
    - Открыть папку (C:\CM)
    
    Все операции требуют прав администратора. PowerShell окно сворачивается.
.NOTES
    Версия: 1.0
    Папка деплоя: C:\CM
    Зависимости: Common-FunctionsV1.ps1, Update-OnlyCertificatesV1.ps1,
                 Install-AvestComponentsV1.ps1, Install-AvCMXWebPV1.ps1,
                 Create-ScheduledTaskV1.ps1, all_certs_urls.txt, avest_urls.ini
    Логи: C:\CM\logs\
#>
...
$updateBtn.Add_Click({
    ...
    try {
        $process = Start-Process -FilePath "powershell.exe" `
            -ArgumentList "-NoProfile -File `"$scriptPath\Update-OnlyCertificatesV1.ps1`"" `
...
$avestBtn.Add_Click({
    ...
    try {
        ...
        if (!(Test-Path $installScript)) {
            throw "Install-AvestComponentsV1.ps1 не найден"
        }
        
        $process = Start-Process -FilePath "powershell.exe" `
            -ArgumentList "-NoProfile -File `"$installScript`" -ScriptPath `"$scriptPath`"" `
...
$avcmxBtn.Add_Click({
    ...
    try {
        ...
        $process = Start-Process -FilePath "powershell.exe" `
            -ArgumentList "-NoProfile -File `"$installScript`" -ScriptPath `"$scriptPath`" -LogsFolder `"$scriptPath\logs`"" `
            -Verb RunAs -Wait -PassThru -WindowStyle Normal
...
$avestInfoBtn.Add_Click({
    # Используем функции из Common-FunctionsV1.ps1 (уже загружена в начале)
    $avPassInstalled = Test-AvPass
    $avBignInstalled = Test-AvBign
    $cspBelInstalled = Test-AvCSPBel
    $cspBignInstalled = Test-AvCSPBign
...
$versionLabel.Text = "Версия 1.0 | Инструмент управления сертификатами"
